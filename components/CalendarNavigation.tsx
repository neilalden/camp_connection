...
const CalendarNavigation = ({ date, setDate }: { date: Date, setDate: SetStateType<Date> }) => {
    ...
    function checkScrollDirection(event: HTMLEvent<any>): "UP" | "DOWN" {
    ...
    return (
        <div className={styles.calendarNavigation}>
            <div className={styles.calendarNavigationYear} onScroll={(e) => {
                const direction = checkScrollDirection(e)
                setDate(new Date(`${months[month]} 01 ${direction === "UP" ? year + 1 : year - 1}`));
            }}>
                {
                    yearItems.map((y, i) => {
            ...
            <div className={styles.calendarNavigationYear}>
                {
                    monthItems.map((m, i) => {
