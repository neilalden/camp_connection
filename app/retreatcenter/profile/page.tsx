...
const Userprofile = () => {
  ...
  const companyLogo = useSelector(
    (state: RootState) => state.RetreatCenter.retreatCenter?.logo
  );
    ...
    <div className={styles.container}>
      <div className={["card", styles.headerContainer].join(" ")}>
        <div className={styles.logoContainer}>
          <Image
            src={companyLogo ? companyLogo : Images.ic_logo} // Use imageUrl or the default image
            alt="Company Logo"
        ...
        <div className={styles.campName}>
          <TextInput
            ...
            value={String(retreatcenter?.name)}
            setValue={setRetreatCenterNameUseState}
            containerClassName={styles.campNameInputStyle}
          />
      ...
      <div className="row-between" style={{ width: "100%" }}>
        ...
        <div className={["card", styles.general].join(" ")}>
          ...
          <form>
            ...
            />
            <CheckBox
              value={isAceeptingGroups}
              onChange={() => setIsAcceptingGroups((prev) => !prev)}
              label="Do you accept groups?"
              containerClassName={styles.inputStyle}
            />
            <CheckBox
              value={isAceeptingRVTent}
              onChange={() => setIsAcceptingRVTent((prev) => !prev)}
              label="Do you accept RV/Tent Camping?"
              containerClassName={styles.inputStyle}
            />
            <CheckBox
              value={isRecievingCCLeads}
              onChange={() => setIsRecievingCCLeads((prev) => !prev)}
              label="Receive Camp Connection Leads?"
              containerClassName={styles.inputStyle}
            />
                  ...
                  <div key={i} className="row-between">
                    ...
                    {sched.editMode ? (
                      <div className={styles.schedContainer}>
                        <div
                          className={["row", styles.timeInputContainer].join(
                            " "
                          )}
                        >
                          ...
                          <input
                            type={"time"}
                            onChange={(e) =>
                              setHour(e.target.value, "from", sched.label)
                            }
                          ...
                          <input
                            type={"time"}
                            onChange={(e) =>
                              setHour(e.target.value, "to", sched.label)
                            }
                            className={styles.timeInput}
                          />
                        </div>
                        <button
                          type="button"
                          className={styles.buttonApplyToAll}
                          onClick={() => applyToAll(sched)}
                        >
