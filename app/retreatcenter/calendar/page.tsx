...
const CalendarPage = () => {
  ...
  const [reservations, setReservations] = useState<Array<ReservationType>>(reservationsSampleData)
    ...
    <div className={styles.container}>
      ...
      <div className={styles.mainRow}>
        ...
        <div className={styles.calendarColumn}>
          ...
          <SimpleCalendar date={date} reservations={reservations} setReservations={setReservations} />
        </div>
      </div>
    </div>
  )
...
const SimpleCalendar = ({ date, reservations, setReservations }: { date: Date, reservations: Array<ReservationType>, setReservations: SetStateType<Array<ReservationType>> }) => {
  const onDrop = (e: React.DragEvent) => {
    ...
    const reservation: ReservationType = {
      ...
      checkOutDate: new Date(new Date().setDate(new Date().getDate() + parsed.checkInDays)),
      status: "Booked"
    }
          ...
          return (
            <div key={i} className={styles.calendarWeek}>
              {calendarDays.map((d, ix) => {
                ...
                const reservation = reservations.find((r) => r.checkInDate && r.checkOutDate && currentDate > r.checkInDate && currentDate <= r.checkOutDate)
                let style: string | undefined;
                console.log("rerendered", reservations)


                if (reservation && reservation.checkInDate && reservation.checkOutDate && currentDate) {
                  let checkInString
                  let checkOutString
                  ...
                  style = [
                    currentDate.getDate() > reservation.checkInDate.getDate() && currentDate.getDate() <= reservation.checkOutDate.getDate() ? styles.booked : styles.calendarDay,
                    reservation.status === "Booked" ? styles.booked : styles.reserved,
                  ].join(" ");
                }
                return (
                  <div key={ix} className={styles.vacant} onDrop={onDrop} onDragOver={dragOver}>
                    {reservation ? <div className={style} onClick={() => alert(JSON.stringify(reservation))} /> : <div className={styles.vacantLine} />}
                  </div>
                )
              })}
            </div>
